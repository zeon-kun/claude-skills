...
# Installs skills and agents globally (~/.claude/) or project-level (./.claude/)
#
# Usage:
#   ./install.sh                              # install all skills + agents globally
...
while [[ $i -lt ${#args[@]} ]]; do
  ...
  case "$arg" in
    ...
    --skills-only) INSTALL_AGENTS=false ;;
    --agents-only) INSTALL_SKILLS=false ;;
    --skills)
